...
public class SoccerTeam implements SportsTeam, Comparable<SoccerTeam>{
	...
	public SoccerTeam(String official, String nick) throws TeamException{
		//Check if parameters are empty strings & initialize or throw exception
		if(official != ""){
			this.officialName = official;
		}
		else{
			throw new TeamException(new String("Official team name cannot be empty."));
		}
		...
		if(nick != ""){
			...
			throw new TeamException(new String("Team nick name cannot be empty."));
		}
	...
	public void playMatch(int goalsFor, int goalsAgainst) throws TeamException{
		//Check goals for, update or throw exception
		if(goalsFor >= 0 && goalsFor <= 20){
			goalsScoredSeason += goalsFor;
		}
		else{
			throw new TeamException(new String("An unrealistic amount of goals was scored by the team."));
		}
		
		//Check goals against, update or throw exception
		if(goalsAgainst >= 0 && goalsAgainst <= 20){
			goalsConcededSeason += goalsAgainst;
		}
		else{
			throw new TeamException("An unrealistic amount of goals was conceded by the team.");
		}
		
		//Update form, matches won, drawn, lost and competition points
		if(goalsFor > goalsAgainst){
			matchesWon++;
			competitionPoints += 3;
			form.addResultToForm(WLD.valueOf("WIN"));
		}
		else if(goalsFor == goalsAgainst){
			matchesDrawn++;
			competitionPoints += 1;
			form.addResultToForm(WLD.valueOf("DRAW"));
		}
		else{
			matchesLost++;
			form.addResultToForm(WLD.valueOf("LOSS"));
		}
	}	
